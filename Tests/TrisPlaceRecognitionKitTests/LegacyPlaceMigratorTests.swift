//
//  LegacyPlaceMigratorTests.swift
//  TrisPlaceRecognitionKit
//
//  Created by COMATOKI on 2026-09-24.
//


import Foundation
import Testing

@testable import TrisPlaceRecognitionKit

struct LegacyPlaceMigratorTests {

    @Test
    func migratesPlacesAndPreservesOriginalFile() async throws {
        let fileURL = makeTemporaryFileURL()

        defer {
            removeTemporaryDirectory(
                for: fileURL
            )
        }

        let gym = try makePlace(
            name: "헬스장"
        )

        let park = try makePlace(
            name: "공원"
        )

        let originalPlaces = [
            gym,
            park
        ]

        try writePlaces(
            originalPlaces,
            to: fileURL
        )

        let originalData = try Data(
            contentsOf: fileURL
        )

        let targetStore = InMemoryPlaceStore()

        let migrator = LegacyPlaceMigrator(
            legacyFileURL: fileURL,
            targetStore: targetStore
        )

        try await migrator.migrate()

        let migratedPlaces = try await targetStore.fetchAll()

        #expect(
            migratedPlaces == originalPlaces
        )

        let currentData = try Data(
            contentsOf: fileURL
        )

        #expect(
            currentData == originalData
        )

        let markerURL = LegacyPlaceMigrator.markerURL(
            for: fileURL
        )

        #expect(
            FileManager.default.fileExists(
                atPath: markerURL.path
            )
        )
    }

    @Test
    func repeatedMigrationDoesNotDuplicatePlaces() async throws {
        let fileURL = makeTemporaryFileURL()

        defer {
            removeTemporaryDirectory(
                for: fileURL
            )
        }

        let place = try makePlace(
            name: "헬스장"
        )

        try writePlaces(
            [place],
            to: fileURL
        )

        let targetStore = InMemoryPlaceStore()

        let migrator = LegacyPlaceMigrator(
            legacyFileURL: fileURL,
            targetStore: targetStore
        )

        try await migrator.migrate()
        try await migrator.migrate()

        let places = try await targetStore.fetchAll()

        #expect(
            places == [place]
        )
    }

    @Test
    func corruptedJSONDoesNotCreateMigrationMarker() async throws {
        let fileURL = makeTemporaryFileURL()

        defer {
            removeTemporaryDirectory(
                for: fileURL
            )
        }

        try FileManager.default.createDirectory(
            at: fileURL.deletingLastPathComponent(),
            withIntermediateDirectories: true
        )

        let corruptedData = Data(
            "invalid JSON".utf8
        )

        try corruptedData.write(
            to: fileURL,
            options: .atomic
        )

        let targetStore = InMemoryPlaceStore()

        let migrator = LegacyPlaceMigrator(
            legacyFileURL: fileURL,
            targetStore: targetStore
        )

        do {
            try await migrator.migrate()

            Issue.record(
                "Expected decoding failure"
            )
        } catch is DecodingError {
        } catch {
            Issue.record(
                "Unexpected error: \(error)"
            )
        }

        let places = try await targetStore.fetchAll()

        #expect(
            places.isEmpty
        )

        let markerURL = LegacyPlaceMigrator.markerURL(
            for: fileURL
        )

        #expect(
            !FileManager.default.fileExists(
                atPath: markerURL.path
            )
        )

        #expect(
            try Data(contentsOf: fileURL) == corruptedData
        )
    }

    @Test
    func conflictingPlaceDoesNotOverwriteExistingData() async throws {
        let fileURL = makeTemporaryFileURL()

        defer {
            removeTemporaryDirectory(
                for: fileURL
            )
        }

        let id = UUID()

        let legacyPlace = try makePlace(
            id: id,
            name: "헬스장"
        )

        let existingPlace = try makePlace(
            id: id,
            name: "공원"
        )

        try writePlaces(
            [legacyPlace],
            to: fileURL
        )

        let targetStore = InMemoryPlaceStore()

        try await targetStore.save(
            existingPlace
        )

        let migrator = LegacyPlaceMigrator(
            legacyFileURL: fileURL,
            targetStore: targetStore
        )

        do {
            try await migrator.migrate()

            Issue.record(
                "Expected conflictingExistingPlace error"
            )
        } catch let error as PlaceMigrationError {
            #expect(
                error == .conflictingExistingPlace(id)
            )
        } catch {
            Issue.record(
                "Unexpected error: \(error)"
            )
        }

        let places = try await targetStore.fetchAll()

        #expect(
            places == [existingPlace]
        )
    }

    @Test
    func detectsChangesToLegacyFileAfterMigration() async throws {
        let fileURL = makeTemporaryFileURL()

        defer {
            removeTemporaryDirectory(
                for: fileURL
            )
        }

        let gym = try makePlace(
            name: "헬스장"
        )

        try writePlaces(
            [gym],
            to: fileURL
        )

        let targetStore = InMemoryPlaceStore()

        let migrator = LegacyPlaceMigrator(
            legacyFileURL: fileURL,
            targetStore: targetStore
        )

        try await migrator.migrate()

        let park = try makePlace(
            name: "공원"
        )

        try writePlaces(
            [gym, park],
            to: fileURL
        )

        do {
            try await migrator.migrate()

            Issue.record(
                "Expected legacyFileChanged error"
            )
        } catch let error as PlaceMigrationError {
            #expect(
                error == .legacyFileChanged
            )
        } catch {
            Issue.record(
                "Unexpected error: \(error)"
            )
        }

        let places = try await targetStore.fetchAll()

        #expect(
            places == [gym]
        )
    }
}

private extension LegacyPlaceMigratorTests {

    func makeTemporaryFileURL() -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent(
                UUID().uuidString,
                isDirectory: true
            )
            .appendingPathComponent(
                "registered-places.json"
            )
    }

    func removeTemporaryDirectory(
        for fileURL: URL
    ) {
        try? FileManager.default.removeItem(
            at: fileURL.deletingLastPathComponent()
        )
    }

    func writePlaces(
        _ places: [RegisteredPlace],
        to fileURL: URL
    ) throws {
        try FileManager.default.createDirectory(
            at: fileURL.deletingLastPathComponent(),
            withIntermediateDirectories: true
        )

        let data = try JSONEncoder().encode(
            places
        )

        try data.write(
            to: fileURL,
            options: .atomic
        )
    }

    func makePlace(
        id: UUID = UUID(),
        name: String
    ) throws -> RegisteredPlace {
        RegisteredPlace(
            id: id,
            name: try PlaceName(name),
            location: PlaceLocation(
                latitude: 37.5665,
                longitude: 126.9780,
                recognitionRadius: 100
            ),
            networkIdentity: PlaceNetworkIdentity(
                ssid: nil,
                bssid: nil
            )
        )
    }
}
